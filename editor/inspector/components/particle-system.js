...
const uiElements = {
            ...
            this.$.resetBounds.addEventListener('confirm', async () => {
                ...
                await Promise.all(componentUUIDs.map(uuid => {
                    return new Promise((res) => {
                        Editor.Message.request(propUtils.getMessageProtocolNodeScene(this.$this), 'execute-component-method', {
                            uuid,
    ...
    showBounds: {
        ready() {
            this.$.showBounds.addEventListener('change', (event) => {
                ...
                componentUUIDs.forEach(uuid => {
                    Editor.Message.send(propUtils.getMessageProtocolNodeScene(this.$this), 'execute-component-method', {
                        uuid,
        ...
        async update() {
            ...
            const values = await Promise.all(
                componentUUIDs.map(
                    uuid => Editor.Message.request(propUtils.getMessageProtocolNodeScene(this.$this), 'execute-component-method', {
                        uuid,
    ...
    noisePreview: {
        async update() {
            ...
            if (!uuid) { return; }
            let data = await Editor.Message.request(propUtils.getMessageProtocolNodeScene(this.$this), 'execute-component-method', {
                uuid,
