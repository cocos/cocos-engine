        ...
        methods: {
            ...
            async updateLODs(operator, index) {
                ...
                if (operator === 'insert') {
                    ...
                    const undoID = await Editor.Message.request(getMessageProtocolNodeScene(that.$el), 'begin-recording', uuid);
                    await Editor.Message.request(getMessageProtocolNodeScene(that.$el), 'lod-insert', uuid, index + 1, (preValue + nextValue) / 2, null);
                    await Editor.Message.request(getMessageProtocolNodeScene(that.$el), 'end-recording', undoID);
                    ...
                    const undoID = await Editor.Message.request(getMessageProtocolNodeScene(that.$el), 'begin-recording', uuid);
                    await Editor.Message.request(getMessageProtocolNodeScene(that.$el), 'lod-erase', uuid, index);
                    await Editor.Message.request(getMessageProtocolNodeScene(that.$el), 'end-recording', undoID);
