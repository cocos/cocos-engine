...
module.exports = {
    ...
    skybox: {
        ...
        enabled: '允许使用设置的立方体贴图来渲染天空盒',
        useIBL: '勾选后，场景物体将使用设置的立方体贴图来进行环境光漫反射及镜面反射计算.',
        useHDR: '切换高/低动态范围模式，每种模式都有自己独立的光源设定。HDR模式会使用光度学灯光单位，配合相机镜头属性进行曝光计算，LDR模式使用无单位光源和无曝光的镜头，更便于保留原图颜色',
        envmap: '设置一个立方体贴图作为环境光源和天空盒. 目前支持十字型、经纬度图、手动创建6张等。支持hdr / tga / png等文件格式.',
    },
    fog: {
        enabled: '雾开关',
        accurate: '切换顶点雾和像素雾。像素雾在顶点数少面积大的物体上有更精确的雾化效果，但是顶点雾有更好的性能，请根据需要选择',
        fogColor: '内散射颜色',
        type: '不同的计算雾化模型',
        fogDensity: '该值越大雾气越浓',
        fogStart: '雾从此距离（世界空间单位）开始，比该距离更近的物体都会完全不受雾的影响',
        fogEnd: '比此距离（世界空间单位）更远的物体，将会接受到最浓的雾',
        fogAtten: '雾的衰减，该值越小则雾越浓',
        fogTop: '比该值（世界空间单位）海拔更高的物体将不会受到雾的影响',
        fogRange: '雾从Top高度向下延伸Range',
    },
    shadow: {
        enabled: '是否开启实时阴影',
        normal: '阴影接收平面的法线',
        distance: '阴影接收平面与原点的距离',
        saturation: '阴影饱和度，该值最好设为1.0，我们建议您通过增加环境光来减小方向光阴影的饱和程度，而不是通过调节该值来完成。',
        pcf: '开启软阴影',
        bias: '增加深度偏移值（世界空间单位）可以有效消除阴影摩尔纹，但是过大的值可能造成漏光现象',
        normalBias: '法线深度偏移值（世界空间单位）可以消除物体表面朝向平行于阳光方向的阴影摩尔纹，但是过大的值可能造成阴影位置偏差',
        shadowMapSize: '阴影贴图分辨率',
        fixedArea: '切换固定区域和CSM模式。固定区域是一种旧模式，我们并不推荐使用。此模式下阴影会跟随方向光节点的位置，在方向光包围盒附近分布，而非跟随相机。',
        near: '固定区域开始值',
        far: '固定区域结束值',
