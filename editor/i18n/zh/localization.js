...
module.exports = {
    ...
    particle_system: {
        ...
        renderer: '粒子渲染模块',
        enableCulling: '选择是否打开粒子剔除功能，开启此功能将会生成一个粒子发射器包围盒，摄像机看不到包围盒将会把此粒子发射器剔除，剔除以后发射器的行为参考 cullingMode 选项',
        cullingMode: '剔除之后的发射器行为，包含选项有 pause、pause and catchup、always simulate；选择 pause 如果相机看不到发射器包围盒粒子暂停模拟，如果再次看到将会以暂停的时间开始继续模拟；选择 pause and catchup 如果相机看不到发射器包围盒粒子暂停模拟，如果再次看到粒子将会以当前的时间开始模拟；选择 always simulate 发射器包围盒无论看到与否都将一直模拟，只是看不到不进行渲染',
        alignSpace: '粒子对齐方向空间，包含选项有视角空间、世界空间、局部空间，选择视角空间粒子模型旋转方向将会跟随摄像机视向，选择世界空间粒子模型的方向将会使用发射器节点的世界空间旋转方向，局部空间粒子使用发射器节点的局部空间旋转方向',
