...
module.exports = {
    ...
    particle_system: {
        ...
        cullingMode: '剔除之后的发射器行为，包含选项有 pause、pause and catchup、always simulate；选择 pause 如果相机看不到发射器包围盒粒子暂停播放，如果再次看到将会以暂停的时间开始继续播放；选择 pause and catchup 如果相机看不到发射器包围盒粒子暂停播放，如果再次看到粒子将会以当前的时间开始播放；选择 always simulate 发射器包围盒无论看到与否都将一直播放，只是看不到不进行渲染',
