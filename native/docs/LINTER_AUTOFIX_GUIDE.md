...
* Open terminal (or git-bash if you are on windows), navigate to `engine-native` folder and run:
...
* For `Clang-Format` issues, there will be no in-place editor suggestions, but all issues are strictly auto-fixable, and if the environment set up above goes smoothly, it is as simple as execute the `Format Document` command. (By default `Shift + Alt + F` on Windows)
    ...
    On the contrary to `Clang-Tidy`, `Clang-Format` auto-formating is a immediate operation and should be used frequently.

> Note: The `compile_commands.json` generated by current approach is essentially the Android development environment. So other platform-specific code may likely not be included in the check. This behavior is consistent with CI test environment.
...
First make sure the first three step in VS Code environment setup are done, i.e. `compile_commands.json` has been generated, then:
