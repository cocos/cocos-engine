...
#if USE_NODE_NAPI
extern napi_status  napi_define_class_with_parent(napi_env                                    env,
                                                  const char *                                utf8name,
                                                  size_t                                      length,
                                                  ...
                                                  napi_value *                                result,
                                                  v8::UniquePersistent<v8::FunctionTemplate> &result_tpl);
extern v8::Isolate *napi_env_isolate(napi_env env);
#endif
...
void Class::install() {
    ...
    auto *                          isolate = napi_env_isolate(env);
    if (_parentProto) {
        //napi_get_reference_value(env, _parentProto->_getClass()->_constructor, &parent);
        parent = _parentProto->_getClass()->_funcTpl.Get(isolate);
    }
    // TODO store function template here!
    NODE_API_CALL(status, env,
                  napi_define_class_with_parent(env, _name.c_str(), -1, _ctorFunc, nullptr, parent, _properties.size(), _properties.data(), &cons, _funcTpl));
    NODE_API_CALL(status, env,
                  napi_create_reference(env, cons, 1, &_constructor));
    NODE_API_CALL(status, env,
                  napi_set_named_property(env, _parent->_getJSObject(), _name.c_str(), cons));
    napi_value proto;
    NODE_API_CALL(status, env, napi_get_named_property(env, cons, "prototype", &proto));
    if (status == napi_ok) {
        _proto = Object::_createJSObject(env, proto, this);
        _proto->root();
    }
#else
