...
static bool js_2d_RenderEntity_getUseLocalNode(se::State& s) // NOLINT(readability-identifier-naming)
{
    auto* cobj = SE_THIS_OBJECT<cc::RenderEntity>(s);
    // SE_PRECONDITION2(cobj, false, "Invalid Native Object");
    if (nullptr == cobj) return true;
    ...
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        cc::Node* result = cobj->getUseLocalNode();
        ok &= nativevalue_to_se(result, s.rval(), nullptr /*ctx*/);
        SE_PRECONDITION2(ok, false, "Error processing arguments");
        SE_HOLD_RETURN_VALUE(result, s.thisObject(), s.rval());
        return true;
...
SE_BIND_FUNC_AS_PROP_GET(js_2d_RenderEntity_getUseLocalNode)
...
static bool js_2d_RenderEntity_setUseLocalNode(se::State& s) // NOLINT(readability-identifier-naming)
{
    auto* cobj = SE_THIS_OBJECT<cc::RenderEntity>(s);
    // SE_PRECONDITION2(cobj, false, "Invalid Native Object");
    if (nullptr == cobj) return true;
    ...
    if (argc == 1) {
        ...
        ok &= sevalue_to_native(args[0], &arg0, s.thisObject());
        SE_PRECONDITION2(ok, false, "Error processing arguments");
        cobj->setUseLocalNode(arg0.value());
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 1);
    return false;
}
SE_BIND_FUNC_AS_PROP_SET(js_2d_RenderEntity_setUseLocalNode)
...
bool js_register_2d_RenderEntity(se::Object* obj) // NOLINT(readability-identifier-naming)
{
    ...
    cls->defineProperty("useLocalNode", _SE(js_2d_RenderEntity_getUseLocalNode_asGetter), _SE(js_2d_RenderEntity_setUseLocalNode_asSetter));
