...
static bool js_mtl_CCMTLDevice_ensureAutoreleasePool(se::State& s)
{
    cc::gfx::CCMTLDevice* cobj = SE_THIS_OBJECT<cc::gfx::CCMTLDevice>(s);
    SE_PRECONDITION2(cobj, false, "js_mtl_CCMTLDevice_ensureAutoreleasePool : Invalid Native Object");
    const auto& args = s.args();
    size_t argc = args.size();
    if (argc == 0) {
        cobj->ensureAutoreleasePool();
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_mtl_CCMTLDevice_ensureAutoreleasePool)
...
bool js_register_mtl_CCMTLDevice(se::Object* obj)
{
    ...
    cls->defineFunction("ensureAutoreleasePool", _SE(js_mtl_CCMTLDevice_ensureAutoreleasePool));
