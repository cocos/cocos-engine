...
{
    cc::network::DownloadTask* cobj = JSB_ALLOC(cc::network::DownloadTask);
...
static bool js_cc_network_DownloadTask_finalize(se::State& s)
{
    auto iter = se::NonRefNativePtrCreatedByCtorMap::find(SE_THIS_OBJECT<cc::network::DownloadTask>(s));
    if (iter != se::NonRefNativePtrCreatedByCtorMap::end())
    {
        se::NonRefNativePtrCreatedByCtorMap::erase(iter);
        cc::network::DownloadTask* cobj = SE_THIS_OBJECT<cc::network::DownloadTask>(s);
        JSB_FREE(cobj);
    }
    return true;
}
