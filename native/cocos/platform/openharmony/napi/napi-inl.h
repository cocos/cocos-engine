...
#define MY_LOGE(...) ((void) OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, "cjh", __VA_ARGS__))
...
inline napi_status AttachData(napi_env env,
  ...
  if (status == napi_ok) {
    ...
    if (status == napi_ok) {
      ...
      char property_name[30] = {0};
      snprintf(property_name, sizeof(property_name), "__ccext_%u", ++name_index);

      napi_property_descriptor desc = {property_name,//nullptr,
                                      nullptr,//  symbol, //cjh
                                       nullptr,
                                       nullptr,
                                       nullptr,
...
struct CallbackData {
  static inline napi_value Wrapper(napi_env env, napi_callback_info info) {
    return details::WrapCallback([&] {
      ...
      auto ret = callbackData->callback(callbackInfo);
      return ret;
    });
  }
...
inline std::string String::Utf8Value() const {
  ...
  status = napi_get_value_string_utf8(
      _env, _value, &value[0], value.capacity(), &length);//cjh add &length
  NAPI_THROW_IF_FAILED(_env, status, "");
  return value;
}
...
inline Function Function::New(napi_env env,
  ...
  CbData* callbackData = new CbData{std::move(cb), data};
...
inline CallbackScope::~CallbackScope() {
  //cjh napi_status status = napi_close_callback_scope(_env, _scope);
  // NAPI_FATAL_IF_FAILED(
...
inline AsyncContext::~AsyncContext() {
  if (_context != nullptr) {
//cjh    napi_async_destroy(_env, _context);
    _context = nullptr;
  }
}
