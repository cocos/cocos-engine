...
T *ConstructN(T *basePtr, size_t count) {
    ...
    #define _CC_MALLOC(bytes) ::cc::gfx::CategorisedAllocPolicy::AllocateBytes(bytes, __FILE__, __LINE__, __FUNCTION__)
    /// Reallocate a block of raw memory.
    #define _CC_REALLOC(ptr, bytes) ::cc::gfx::CategorisedAllocPolicy::ReallocateBytes(ptr, bytes, __FILE__, __LINE__, __FUNCTION__)
    /// Allocate a block of memory for a primitive type.
    #define _CC_ALLOC_T(T, count) static_cast<T *>(::cc::gfx::CategorisedAllocPolicy::AllocateBytes(sizeof(T) * (count), __FILE__, __LINE__, __FUNCTION__))
    /// Free the memory allocated with _CC_MALLOC or _CC_ALLOC_T.
    #define _CC_FREE(ptr) ::cc::gfx::CategorisedAllocPolicy::DeallocateBytes((void *)ptr)

    /// Allocate space for one primitive type, external type or non-virtual type with constructor parameters
    #define _CC_NEW_T(T) new (::cc::gfx::CategorisedAllocPolicy::AllocateBytes(sizeof(T), __FILE__, __LINE__, __FUNCTION__)) T
    /// Allocate a block of memory for 'count' primitive types - do not use for classes that inherit from AllocatedObject
    #define _CC_NEW_ARRAY_T(T, count) ::cc::gfx::ConstructN(static_cast<T *>(::cc::gfx::CategorisedAllocPolicy::AllocateBytes(sizeof(T) * (count), __FILE__, __LINE__, __FUNCTION__)), count)
    /// Free the memory allocated with _CC_NEW_T.
    #define _CC_DELETE_T(ptr, T)                                        \
        if (ptr) {                                                      \
            (ptr)->~T();                                                \
            ::cc::gfx::CategorisedAllocPolicy::DeallocateBytes((void *)ptr); \
        }
    ...
    #define _CC_MALLOC_ALIGN(bytes, align) ::cc::gfx::CategorisedAllocPolicy::AllocateBytesAligned(align, bytes, __FILE__, __LINE__, __FUNCTION__)
    /// Allocate a block of memory for a primitive type aligned to user defined boundaries.
    #define _CC_ALLOC_T_ALIGN(T, count, align) static_cast<T *>(::cc::gfx::CategorisedAllocPolicy::AllocateBytesAligned(align, sizeof(T) * (count), __FILE__, __LINE__, __FUNCTION__))
    /// Free the memory allocated with either _CC_MALLOC_ALIGN or _CC_ALLOC_T_ALIGN.
    #define _CC_FREE_ALIGN(ptr, align) ::cc::gfx::CategorisedAllocPolicy::DeallocateBytesAligned(ptr)
    ...
    #define _CC_NEW_T_ALIGN(T, align) new (::cc::gfx::CategorisedAllocPolicy::AllocateBytesAligned(align, sizeof(T), __FILE__, __LINE__, __FUNCTION__)) T
    ...
    #define _CC_NEW_ARRAY_T_ALIGN(T, count, align) ::cc::gfx::ConstructN(static_cast<T *>(::cc::gfx::CategorisedAllocPolicy::AllocateBytesAligned(align, sizeof(T) * (count), __FILE__, __LINE__, __FUNCTION__)), count)
    /// Free the memory allocated with _CC_NEW_T_ALIGN.
    #define _CC_DELETE_T_ALIGN(ptr, T, align)                          \
        if (ptr) {                                                     \
            (ptr)->~T();                                               \
            ::cc::gfx::CategorisedAllocPolicy::DeallocateBytesAligned(ptr); \
        }
    ...
    /// Allocate a block of raw memory.
    #define _CC_MALLOC(bytes)         ::cc::gfx::CategorisedAllocPolicy::AllocateBytes(bytes)
    /// Reallocate a block of raw memory.
    #define _CC_REALLOC(ptr, bytes)   ::cc::gfx::CategorisedAllocPolicy::ReallocateBytes(ptr, bytes)
    /// Allocate a block of memory for a primitive type.
    #define _CC_ALLOC_T(T, count)     static_cast<T *>(::cc::gfx::CategorisedAllocPolicy::AllocateBytes(sizeof(T) * (count)))
    ...
    /// Allocate space for one primitive type, external type or non-virtual type with constructor parameters
    #define _CC_NEW_T(T)              new (::cc::gfx::CategorisedAllocPolicy::AllocateBytes(sizeof(T))) T
    /// Allocate a block of memory for 'count' primitive types - do not use for classes that inherit from AllocatedObject
    #define _CC_NEW_ARRAY_T(T, count) ::cc::gfx::ConstructN(static_cast<T *>(::cc::gfx::CategorisedAllocPolicy::AllocateBytes(sizeof(T) * (count))), count)
    /// Free the memory allocated with _CC_NEW_T.
    #define _CC_DELETE_T(ptr, T)                                        \
        if (ptr) {                                                      \
            (ptr)->~T();                                                \
    ...
    // aligned allocation
    /// Allocate a block of raw memory aligned to user defined boundaries.
    #define _CC_MALLOC_ALIGN(bytes, align)         ::cc::gfx::CategorisedAllocPolicy::AllocateBytesAligned(align, bytes)
    /// Allocate a block of memory for a primitive type aligned to user defined boundaries.
    #define _CC_ALLOC_T_ALIGN(T, count, align)     static_cast<T *>(::cc::gfx::CategorisedAllocPolicy::AllocateBytesAligned(align, sizeof(T) * (count)))
    /// Free the memory allocated with either _CC_MALLOC_ALIGN or _CC_ALLOC_T_ALIGN.
    #define _CC_FREE_ALIGN(ptr, align)             ::cc::gfx::CategorisedAllocPolicy::DeallocateBytesAligned((void *)ptr)
    /// Allocate a block of raw memory aligned to SIMD boundaries.
    #define _CC_MALLOC_SIMD(bytes)                 _CC_MALLOC_ALIGN(bytes, CC_SIMD_ALIGNMENT)
    /// Allocate a block of memory for a primitive type aligned to SIMD boundaries.
    #define _CC_ALLOC_T_SIMD(T, count)             _CC_ALLOC_T_ALIGN(T, count, CC_SIMD_ALIGNMENT)
    ...
    /// Allocate space for one primitive type, external type or non-virtual type aligned to user defined boundaries
    #define _CC_NEW_T_ALIGN(T, align)              new (::cc::gfx::CategorisedAllocPolicy::AllocateBytesAligned(align, sizeof(T))) T
    /// Allocate a block of memory for 'count' primitive types aligned to user defined boundaries - do not use for classes that inherit from AllocatedObject
    #define _CC_NEW_ARRAY_T_ALIGN(T, count, align) ::cc::gfx::ConstructN(static_cast<T *>(::cc::gfx::CategorisedAllocPolicy::AllocateBytesAligned(align, sizeof(T) * (count))), count)
    /// Free the memory allocated with _CC_NEW_T_ALIGN.
    #define _CC_DELETE_T_ALIGN(ptr, T, align)                                  \
        if (ptr) {                                                             \
            (ptr)->~T();                                                       \
            ::cc::gfx::CategorisedAllocPolicy::DeallocateBytesAligned((void *)ptr); \
        }
    /// Free the memory allocated with _CC_NEW_ARRAY_T_ALIGN.
    #define _CC_DELETE_ARRAY_T_ALIGN(ptr, T, count, align)                     \
        if (ptr) {                                                             \
