...
class Setter {
public:
    Setter(RenderData& data) noexcept // NOLINT
    : mData(data) {}
...
class RasterQueue final : public Setter {
public:
    RasterQueue(RenderData& data, RenderGraph& renderGraph, uint32_t vertID, RenderQueueData& queue, pipeline::PipelineSceneData& pipeline) noexcept
    : Setter(data),
    ...
    RenderQueueData&             mQueue;
    pipeline::PipelineSceneData& mPipeline;
};

class RasterPass final : public Setter {
public:
    RasterPass(RenderData& data, RenderGraph& renderGraph, uint32_t vertID, RasterPassData& pass, pipeline::PipelineSceneData& pipeline) noexcept
    : Setter(data),
      mRenderGraph(renderGraph),
      mVertID(vertID),
      mPass(pass),
    ...
    RenderGraph&                 mRenderGraph;
    const uint32_t               mVertID = 0xFFFFFFFF;
    RasterPassData&              mPass;
    pipeline::PipelineSceneData& mPipeline;
};

class ComputeQueue final : public Setter {
public:
    ComputeQueue(RenderData& data, RenderGraph& renderGraph, uint32_t vertID, RenderQueueData& queue, pipeline::PipelineSceneData& pipeline) noexcept
    : Setter(data),
      mRenderGraph(renderGraph),
      mVertID(vertID),
      mQueue(queue),
...
class ComputePass final : public Setter {
public:
    ComputePass(RenderData& data, RenderGraph& renderGraph, uint32_t vertID, ComputePassData& pass, pipeline::PipelineSceneData& pipeline) noexcept
    : Setter(data),
      mRenderGraph(renderGraph),
      mVertID(vertID),
      mPass(pass),
    ...
    RenderGraph&                 mRenderGraph;
    const uint32_t               mVertID = 0xFFFFFFFF;
    ComputePassData&             mPass;
    pipeline::PipelineSceneData& mPipeline;
};

struct MovePass {
    MovePass(RenderGraph& renderGraph, uint32_t vertID, MovePassData& pass) noexcept
    : mRenderGraph(renderGraph),
      mVertID(vertID),
      mPass(pass) {}
...
struct CopyPass {
    CopyPass(RenderGraph& renderGraph, uint32_t vertID, CopyPassData& pass) noexcept
    : mRenderGraph(renderGraph),
      mVertID(vertID),
      mPass(pass) {}
