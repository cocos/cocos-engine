...
AudioResampler *AudioResampler::create(audio_format_t format, int inChannelCount,
    ...
    switch (quality) {
        ...
        case LOW_QUALITY:
            ...
            resampler = ccnew (std::nothrow) AudioResamplerOrder1(inChannelCount, sampleRate);
            break;
        case MED_QUALITY:
            ALOGV("Create cubic Resampler");
            LOG_ALWAYS_FATAL_IF(format != AUDIO_FORMAT_PCM_16_BIT, "invalid pcm format");
            resampler = ccnew (std::nothrow) AudioResamplerCubic(inChannelCount, sampleRate);
            break;
        case HIGH_QUALITY:
            ...
            //        resampler = ccnew (std::nothrow) AudioResamplerSinc(inChannelCount, sampleRate);
            break;
        case VERY_HIGH_QUALITY:
            ALOGV("Create VERY_HIGH_QUALITY sinc Resampler = %d", quality);
            LOG_ALWAYS_FATAL_IF(format != AUDIO_FORMAT_PCM_16_BIT, "invalid pcm format");
            // Cocos2d-x only uses MED_QUALITY, so we could remove Sinc relative files
            //        resampler = ccnew (std::nothrow) AudioResamplerSinc(inChannelCount, sampleRate, quality);
