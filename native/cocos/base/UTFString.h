...
public:
    ...
    protected:
        ...
        inline void _become(const _base_iterator &i) {
            iter_ = i.iter_;
            string_ = i.string_;
        }
    ...
    UTFString(const String &str) {
        _init();
        assign(str);
    }
    //! duplicate of \a wstr
    UTFString(const WString &wstr) {
        _init();
        assign(wstr);
    }
   ...
   only change the string length by + or - 1. */
    ...
    inline UTFString &append(const UTFString &str) {
        m_data.append(str.m_data);
        return *this;
    }
    //! appends \a str on to the end of the current string
    inline UTFString &append(const code_point *str) {
        m_data.append(str);
        return *this;
    }
    //! appends a substring of \a str starting at \a index that is \a len characters long on to the end of the current string
    inline UTFString &append(const UTFString &str, size_type index, size_type len) {
        m_data.append(str.m_data, index, len);
        return *this;
    }
    //! appends \a num characters of \a str on to the end of the current string
    inline UTFString &append(const code_point *str, size_type num) {
        m_data.append(str, num);
        return *this;
    }
    //! appends \a num repetitions of \a ch on to the end of the current string
    inline UTFString &append(size_type num, code_point ch) {
        m_data.append(num, ch);
        return *this;
    }
    //! appends the sequence denoted by \a start and \a end on to the end of the current string
    inline UTFString &append(iterator start, iterator end) {
        m_data.append(start.iter_, end.iter_);
        return *this;
    }
#if CC_IS_NATIVE_WCHAR_T
    //! appends \a num characters of \a str on to the end of the current string
    inline UTFString &append(const wchar_t *w_str, size_type num) {
        WString tmp(w_str, num);
        return append(tmp);
    }
    //! appends \a num repetitions of \a ch on to the end of the current string
