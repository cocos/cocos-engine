...
export function setupBuildTimeConstants ({
    ...
    result.RUNTIME_BASED = result.OPPO || result.VIVO || result.HUAWEI || result.COCOSPLAY || result.LINKSURE || result.QTT || result.SUD || result.SUDV2;
...
export function getPlatformConstantNames () {
    return ['HTML5', 'WECHAT', 'ALIPAY', 'BAIDU', 'XIAOMI', 'BYTEDANCE', 'OPPO', 'VIVO', 'HUAWEI', 'NATIVE', 'COCOSPLAY', 'LINKSURE', 'QTT', 'SUD', 'SUDV2', 'NODEJS'];
}
