...
test('simple spriteQuad trimmed', function () {
    var testCallBack = function () {
        ...
        strictEqual(vertices.length >= 8 && uvs >= 8, true, 'vertices have at least 8 floats');
        ...
        deepClose(uvs[0], 10 / 100, 0.0001, 'texCoords-bl-x test success');
        deepClose(uvs[1], 70 / 100, 0.0001, 'texCoords-bl-y test success');

        deepClose(uvs[6], 50 / 100, 0.0001, 'texCoords-tr-x test success');
        deepClose(uvs[7], 10 / 100, 0.0001, 'texCoords-tr-y test success');
    };
...
test('scale9 spriteQuad trimmed', function () {
    var testCallBack = function () {
        ...
        var vertices = s9Sprite._vertices;
        var uvs = s9Sprite._uvs;
        strictEqual(vertices.length >= 32 && uvs >= 32, true, 'vertices have at least 32 floats');
        ...
        deepClose(uvs[0], 10 / 100, 0.0001, 'u0 test success');
        deepClose(uvs[1], 70 / 100, 0.0001, 'v0 test success');
        deepClose(uvs[10], 15 / 100, 0.0001, 'u1 test success');
        deepClose(uvs[11], 63 / 100, 0.0001, 'v1 test success');
        deepClose(uvs[20], 44 / 100, 0.0001, 'u2 test success');
        deepClose(uvs[21], 18 / 100, 0.0001, 'v2 test success');
        deepClose(uvs[30], 50 / 100, 0.0001, 'u3 test success');
        deepClose(uvs[31], 10 / 100, 0.0001, 'v3 test success');
    };
    if (spriteFrame.textureLoaded()) {
        testCallBack();
    } else {
...
test('tiled spriteQuad trimmed', function () {
    var testCallBack = function () {
        ...
        strictEqual(vertices[0], 0, 'x0 test success');
        strictEqual(vertices[16], 40, 'x1 test success');
        strictEqual(vertices[32], 80, 'x2 test success');
        strictEqual(vertices[38], 100, 'x3 test success');

        strictEqual(vertices[1], 0, 'y0 test success');
        strictEqual(vertices[41], 60, 'y1 test success');
        strictEqual(vertices[47], 100, 'y2 test success');
        //
        deepClose(uvs[16], 10 / 100, 0.0001, 'full quad u0 test success');
        deepClose(uvs[17], 70 / 100, 0.0001, 'full quad v0 test success');
        deepClose(uvs[22], 50 / 100, 0.0001, 'full quad u1 test success');
        deepClose(uvs[23], 10 / 100, 0.0001, 'full quad v1 test success');
        deepClose(uvs[40], 10 / 100, 0.0001, 'part quad u0 test success');
        deepClose(uvs[41], 70 / 100, 0.0001, 'part quad v0 test success');
        deepClose(uvs[46], 30 / 100, 0.0001, 'part quad u1 test success');
        deepClose(uvs[47], 30 / 100, 0.0001, 'part quad v1 test success');
    };
    if (spriteFrame.textureLoaded()) {
        testCallBack();
    } else {
...
test('filled-bar-horizontal spriteQuad trimmed', function () {
    var testCallBack = function () {
        ...
        deepClose(uvs[0], 14 / 100, 0.0001, 'texCoords-bl-x test success');
        deepClose(uvs[1], 70 / 100, 0.0001, 'texCoords-bl-y test success');
        deepClose(uvs[6], 26 / 100, 0.0001, 'texCoords-tr-x test success');
        deepClose(uvs[7], 10 / 100, 0.0001, 'texCoords-tr-y test success');
        ...
        uvs = s9Sprite._uvs;
        strictEqual(vertices.length >= 8 && uvs >= 8, true, 'vertices have at least 8 floats');
        strictEqual(vertices[0]._bl.vertices.x, 0, 'vertex-bl-x test success');
        strictEqual(vertices[1]._bl.vertices.y, 0, 'vertex-bl-y test success');
        strictEqual(vertices[6]._tr.vertices.x, 30, 'vertex-tr-x test success');
        strictEqual(vertices[7]._tr.vertices.y, 100, 'vertex-tr-y test success');

        deepClose(uvs[0]._bl.texCoords.u, 10 / 100, 0.0001, 'texCoords-bl-x test success');
        deepClose(uvs[1]._bl.texCoords.v, 70 / 100, 0.0001, 'texCoords-bl-y test success');
        deepClose(uvs[6]._tr.texCoords.u, 22 / 100, 0.0001, 'texCoords-tr-x test success');
        deepClose(uvs[7]._tr.texCoords.v, 10 / 100, 0.0001, 'texCoords-tr-y test success');
    };
    if (spriteFrame.textureLoaded()) {
        testCallBack();
    } else {
...
test('filled-bar-vertical spriteQuad trimmed', function () {
    var testCallBack = function () {
        ...
        deepClose(uvs[0], 10 / 100, 0.0001, 'texCoords-bl-x test success');
        deepClose(uvs[1], 64 / 100, 0.0001, 'texCoords-bl-y test success');
        deepClose(uvs[6], 50 / 100, 0.0001, 'texCoords-tr-x test success');
        deepClose(uvs[7], 46 / 100, 0.0001, 'texCoords-tr-y test success');
        ...
        deepClose(uvs[1], 70 / 100, 0.0001, 'texCoords-bl-y test success');
        deepClose(uvs[6], 50 / 100, 0.0001, 'texCoords-tr-x test success');
        deepClose(uvs[7], 52 / 100, 0.0001, 'texCoords-tr-y test success');
    };
    if (spriteFrame.textureLoaded()) {
        testCallBack();
    } else {
...
test('filled-bar-radial spriteQuad trimmed', function () {
    var testCallBack = function () {
        ...
        strictEqual(vertices.length >= 30 && uvs >= 30, true, 'vertices have at least 30 floats');
        ...
        deepClose(vertices[6], center.x, 0.0001, 'center test success');
        deepClose(vertices[7], center.y, 0.0001, 'center test success');
        deepClose(vertices[2], p0.x, 0.0001, 'p0 test success');
        deepClose(vertices[3], p0.y, 0.0001, 'p0 test success');
        deepClose(vertices[8], p1.x, 0.0001, 'p1 test success');
        deepClose(vertices[9], p1.y, 0.0001, 'p1 test success');
        deepClose(vertices[10], p2.x, 0.0001, 'p2 test success');
        deepClose(vertices[11], p2.y, 0.0001, 'p2 test success');
    };
    if (spriteFrame.textureLoaded()) {
        testCallBack();
    } else {
