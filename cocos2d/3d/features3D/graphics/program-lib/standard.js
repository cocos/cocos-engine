cc3d.programlib.standard = {
    ...
    _correctChannel: function (p, chan) {
        if (cc3d._matTex2D[p] > 0) {
            if (cc3d._matTex2D[p] < chan.length) {
                return chan.substring(0, cc3d._matTex2D[p]);
            } else if (cc3d._matTex2D[p] > chan.length) {
                var str = chan;
                var chr = str.charAt(str.length - 1);
                var addLen = cc3d._matTex2D[p] - str.length;
    ...
    _nonPointShadowMapProjection: function (light, shadowCoordArgs) {
        if (!light.getNormalOffsetBias() || light._shadowType > cc3d.SHADOW_DEPTH) {
            if (light.getType() === cc3d.LIGHTTYPE_SPOT) {
    ...
    createShaderDefinition: function (device, options) {
        ...
        if (options.shadingModel === cc3d.SPECULAR_PHONG) {
            ...
            options.fresnelModel = (options.fresnelModel === 0) ? cc3d.FRESNEL_SCHLICK : options.fresnelModel;
        }
        ...
        var useTangents = cc3d.precalculatedTangents;
        ...
        if (!options.noShadow) {
            for (i = 0; i < options.lights.length; i++) {
                ...
                if (options.lights[i].getCastShadows()) {
                    if (lightType === cc3d.LIGHTTYPE_DIRECTIONAL) {
                        code += "uniform mat4 light" + i + "_shadowMatrixVS;\n";
                        code += "uniform vec3 light" + i + "_shadowParamsVS;\n";
                        code += "uniform vec3 light" + i + (lightType === cc3d.LIGHTTYPE_DIRECTIONAL ? "_directionVS" : "_positionVS") + ";\n";
                        mainShadowLight = i;
                        break;
                    }
                }
        ...
        if (needsNormal) {
            attributes.vertex_normal = cc3d.SEMANTIC_NORMAL;
            ...
            if ((options.heightMap || options.normalMap) && useTangents) {
                attributes.vertex_tangent = cc3d.SEMANTIC_TANGENT;
        ...
        for (i = 0; i < maxUvSets; i++) {
            if (useUv[i]) {
                attributes["vertex_texCoord" + i] = cc3d["SEMANTIC_TEXCOORD" + i];
                code += chunks["uv" + i + "VS"];
        ...
        code += options.forceFragmentPrecision ? "precision " + options.forceFragmentPrecision + " float;\n\n" : cc3d.programlib.precisionCode(device);
        ...
        for (i = 0; i < options.lights.length; i++) {
            ...
            if (lightType === cc3d.LIGHTTYPE_DIRECTIONAL) {
                ...
                if (lightType === cc3d.LIGHTTYPE_SPOT) {
                    code += "uniform vec3 light" + i + "_direction;\n";
            ...
            if (light.getCastShadows() && !options.noShadow) {
                ...
                if (lightType === cc3d.LIGHTTYPE_POINT) {
                ...
                if (light._shadowType > cc3d.SHADOW_DEPTH) useVsm = true;
            }
            if (light._cookie) {
        ...
        code += cc3d.programlib.gammaCode(options.gamma);
        code += cc3d.programlib.tonemapCode(options.toneMap);
        ...
        if (options.blendType !== cc3d.BLEND_NONE || options.alphaTest) {
        ...
        if (options.useSpecular) {
            ...
            if (options.fresnelModel > 0) {
                if (options.fresnelModel === cc3d.FRESNEL_SIMPLE) {
                    code += chunks.fresnelSimplePS;
                } else if (options.fresnelModel === cc3d.FRESNEL_SCHLICK) {
                    code += chunks.fresnelSchlickPS;
                } else if (options.fresnelModel === cc3d.FRESNEL_COMPLEX) {
        ...
        if (useAo) {
            ...
            if (options.occludeSpecular) {
                if (options.occludeSpecular === cc3d.SPECOCC_AO) {
        ...
        if (options.useSpecular) {
            code += options.shadingModel === cc3d.SPECULAR_PHONG ? chunks.lightSpecularPhongPS : chunks.lightSpecularBlinnPS;
        ...
        if (lighting || reflections) {
            ...
            for (i = 0; i < options.lights.length; i++) {
                ...
                if (lightType === cc3d.LIGHTTYPE_DIRECTIONAL) {
                    ...
                    if (light._cookie) {
                        if (lightType === cc3d.LIGHTTYPE_SPOT && !light._cookie._cubemap) {
                            usesCookie = true;
                            usesCookieNow = true;
                        } else if (lightType === cc3d.LIGHTTYPE_POINT && light._cookie._cubemap) {
                            usesCookie = true;
                            usesCookieNow = true;
                        }
                    }
                    ...
                    if (light.getFalloffMode() == cc3d.LIGHTFALLOFF_LINEAR) {
                ...
                if (light.getCastShadows() && !options.noShadow) {
                    ...
                    var evsmExp;
                    if (light._shadowType === cc3d.SHADOW_VSM8) {
                        shadowReadMode = "VSM8";
                        evsmExp = "0.0";
                    } else if (light._shadowType === cc3d.SHADOW_VSM16) {
                        shadowReadMode = "VSM16";
                        evsmExp = "5.54";
                    } else if (light._shadowType === cc3d.SHADOW_VSM32) {
                    ...
                    } else if (options.shadowSampleType === cc3d.SHADOWSAMPLE_HARD) {
                        shadowReadMode = "Hard";
                    } else if (options.shadowSampleType === cc3d.SHADOWSAMPLE_PCF3X3) {
                    ...
                    if (shadowReadMode !== null) {
                        if (lightType === cc3d.LIGHTTYPE_POINT) {
                            ...
                            if (lightType === cc3d.LIGHTTYPE_SPOT) shadowReadMode = "Spot" + shadowReadMode;
                            code += "       dAtten *= getShadow" + shadowReadMode + "(light" + i + "_shadowMap, light" + i + "_shadowParams"
                                + (light._shadowType > cc3d.SHADOW_DEPTH ? ", " + evsmExp : "") + ");\n";
                        }
                    }
                }
        ...
        code += chunks.endPS;
        if (options.blendType === cc3d.BLEND_NORMAL || options.blendType === cc3d.BLEND_ADDITIVEALPHA) {
            code += chunks.outputAlphaPS;
        } else if (options.blendType === cc3d.BLEND_PREMULTIPLIED) {
