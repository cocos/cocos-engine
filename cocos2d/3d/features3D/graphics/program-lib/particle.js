...
cc3d.programlib.particle = {
    ...
    createShaderDefinition: function (device, options) {
        ...
        var chunk = cc3d.shaderChunks;

        var vshader = "";
        var fshader = cc3d.programlib.precisionCode(device) + "\n";
        ...
        fshader += cc3d.programlib.gammaCode(options.gamma);
        fshader += cc3d.programlib.tonemapCode(options.toneMap);
        ...
        if (options.blend == cc3d.BLEND_NORMAL) {
            fshader += chunk.particle_blendNormalPS;
        } else if (options.blend == cc3d.BLEND_ADDITIVE) {
            fshader += chunk.particle_blendAddPS;
        } else if (options.blend == cc3d.BLEND_MULTIPLICATIVE) {
            fshader += chunk.particle_blendMultiplyPS;
        }
        fshader += chunk.particle_endPS;

        var attributes = cc3d.shaderChunks.collectAttribs(vshader);
