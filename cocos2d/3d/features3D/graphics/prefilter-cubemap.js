cc3d.extend(cc3d, (function () {
    ...
    function syncToCpu(device, targ, face) {
        ...
        if (tex.format != cc3d.PIXELFORMAT_R8_G8_B8_A8) return;
    ...
    function prefilterCubemap(options) {
        ...
        var rgbFormat = format === cc3d.PIXELFORMAT_R8_G8_B8;
        var isImg = false;
        ...
        if ((rgbFormat || isImg) && cpuSync) {
            ...
            nextCubemap = new cc3d.gfx.Texture(device, {
                cubemap: true,
                rgbm: rgbmSource,
                format: format,
                width: size,
            ...
            nextCubemap.minFilter = cc3d.FILTER_LINEAR;
            nextCubemap.magFilter = cc3d.FILTER_LINEAR;
            nextCubemap.addressU = cc3d.ADDRESS_CLAMP_TO_EDGE;
            nextCubemap.addressV = cc3d.ADDRESS_CLAMP_TO_EDGE;
            for (face = 0; face < 6; face++) {
                targ = new cc3d.RenderTarget(device, nextCubemap, {
                    face: face,
                    depth: false
                });
                params.x = face;
                ...
                cc3d.drawQuadWithShader(device, targ, shader2);
        ...
        for (i = 0; i < numMips; i++) {
            for (pass = startPass; pass < cmapsList.length; pass++) {
                if (cmapsList[pass] != null) {
                    cmapsList[pass][i] = new cc3d.gfx.Texture(device, {
                        cubemap: true,
                        rgbm: pass < 2 ? rgbmSource : true,
                        format: pass < 2 ? format : cc3d.PIXELFORMAT_R8_G8_B8_A8,
                        fixCubemapSeams: pass === 1 || pass === 3,
                        width: mipSize[i],
                        height: mipSize[i],
                        autoMipmap: false
                    });
                    cmapsList[pass][i].minFilter = cc3d.FILTER_LINEAR;
                    cmapsList[pass][i].magFilter = cc3d.FILTER_LINEAR;
                    cmapsList[pass][i].addressU = cc3d.ADDRESS_CLAMP_TO_EDGE;
                    cmapsList[pass][i].addressV = cc3d.ADDRESS_CLAMP_TO_EDGE;
                }
            }
        }
        ...
        for (pass = startPass; pass < cmapsList.length; pass++) {
            if (cmapsList[pass] != null) {
                ...
                for (i = 0; i < numMips; i++) {
                    for (face = 0; face < 6; face++) {
                        targ = new cc3d.RenderTarget(device, cmapsList[pass][i], { // TODO: less excessive allocations
                            face: face,
                            depth: false
                        });
                        params.x = face;
        ...
        if (cpuSync && options.singleFilteredFixed) {
            ...
            cubemap = new cc3d.gfx.Texture(device, {
                cubemap: true,
                rgbm: rgbmSource,
            ...
            cubemap.upload();
            cubemap.minFilter = cc3d.FILTER_LINEAR_MIPMAP_LINEAR;
            cubemap.magFilter = cc3d.FILTER_LINEAR;
    ...
    function shFromCubemap(source, dontFlipX) {
        ...
        if (source.format != cc3d.PIXELFORMAT_R8_G8_B8_A8) {
        ...
        if (!source._levels[0][0].length) {
            ...
            if (source._levels[0][0] instanceof HTMLImageElement) {
                ...
                for (face = 0; face < 6; face++) {
                    ...
                    var tex2 = new cc3d.gfx.Texture(device, {
                        cubemap: false,
                        rgbm: false,
                        format: source.format,
                        width: cubeSize,
                    ...
                    var targ = new cc3d.RenderTarget(device, tex2, {
                        depth: false
                    });
