cc3d.extend(cc3d, function () {
     ...
     * @param {cc3d.GraphicsDevice} device Graphics device used to manage internal graphics resources.
     * @param {Number} width The width of the pick buffer in pixels.
     * @param {Number} height The height of the pick buffer in pixels.
     * @property {Number} width Width of the pick buffer in pixels (read-only).
     * @property {Number} height Height of the pick buffer in pixels (read-only).
     * @property {cc3d.RenderTarget} renderTarget The render target used by the picker internally (read-only).
     */
     ...
     * @returns {cc3d.MeshInstance[]} An array of mesh instances that are in the selection
     * @example
    ...
    /**
     * @function
     * @name cc3d.Picker#prepare
     * @description Primes the pick buffer with a rendering of the specified models from the point of view
     * of the supplied camera. Once the pick buffer has been prepared, cc3d.Picker#getSelection can be
     * called multiple times on the same picker object. Therefore, if the models or camera do not change
     * in any way, cc3d.Picker#prepare does not need to be called again.
     * @param {cc3d.Camera} camera The camera used to render the scene, note this is the CameraNode, not an Entity
     * @param {cc3d.Scene} scene The scene containing the pickable mesh instances.
     */
    Picker.prototype.prepare = function (camera, scene) {
        ...
        for (i = 0; i < this.drawCalls.length; i++) {
            if (this.drawCalls[i].command) {
                ...
                type = mesh.primitive[cc3d.RENDERSTYLE_SOLID].type;
                var isSolid = (type === cc3d.PRIMITIVE_TRIANGLES) || (type === cc3d.PRIMITIVE_TRISTRIP) || (type === cc3d.PRIMITIVE_TRIFAN);
                var isPickable = (material instanceof cc3d.StandardMaterial) || (material instanceof cc3d.BasicMaterial);
                if (isSolid && isPickable) {
                    ...
                    shader = meshInstance._shader[cc3d.SHADER_PICK];
                    if (!shader) {
                        ...
                        meshInstance._shader[cc3d.SHADER_PICK] = shader;
                    }
                    device.setShader(shader);

                    device.setVertexBuffer(mesh.vertexBuffer, 0);
                    device.setIndexBuffer(mesh.indexBuffer[cc3d.RENDERSTYLE_SOLID]);
                    device.draw(mesh.primitive[cc3d.RENDERSTYLE_SOLID]);
                }
            }
        }
     ...
     * results returned by cc3d.Picker#getSelection. On the other hand, smaller pick buffers will
     * yield greater performance, so there is a trade off.
     * @param {Number} width The width of the pick buffer in pixels.
     * @param {Number} height The height of the pick buffer in pixels.
     */
    Picker.prototype.resize = function (width, height) {
        var colorBuffer = new cc3d.Texture(this.device, {
        ...
        this._pickBufferTarget = new cc3d.RenderTarget(this.device, colorBuffer, {depth: true});
    };
