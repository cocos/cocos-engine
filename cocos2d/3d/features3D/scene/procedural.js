/**
 * @function
 * @name cc3d.calculateNormals
 * @description Generates normal information from the specified positions and triangle indices. See {@link cc3d.createMesh}.
 ...
 * @example
 * var normals = cc3d.calculateNormals(positions, indices);
 * var tangents = cc3d.calculateTangents(positions, normals, uvs, indices);
 * var mesh = cc3d.createMesh(positions, normals, tangents, uvs, indices);
...
cc3d.calculateNormals = function (positions, indices) {
    ...
    var p2 = new cc.Vec3();
    var p3 = new cc.Vec3();
    var p1p2 = new cc.Vec3();
    var p1p3 = new cc.Vec3();
    var faceNormal = new cc.Vec3();
    var vertexNormal = new cc.Vec3();
 ...
 * @author Will Eastcott
 */
cc3d.calculateTangents = function (positions, normals, uvs, indices) {
    var triangleCount = indices.length / 3;
    var vertexCount = positions.length / 3;
    var i1, i2, i3;
    var x1, x2, y1, y2, z1, z2, s1, s2, t1, t2, r;
    var sdir = new cc.Vec3();
    var tdir = new cc.Vec3();
    var v1 = new cc.Vec3();
    var v2 = new cc.Vec3();
    var v3 = new cc.Vec3();
    var w1 = new cc.Vec2v2();
    var w2 = new cc.Vec2v2();
    var w3 = new cc.Vec2v2();
    var i; // Loop counter
...
/**
 * @function
 * @name cc3d.createMesh
 * @description Creates a new mesh object from the supplied vertex information and topology.
 * @param {cc3d.GraphicsDevice} device The graphics device used to manage the mesh.
 * @param {Number[]} positions An array of 3-dimensional vertex positions.
 ...
 * @returns {cc3d.Mesh} A new Geometry constructed from the supplied vertex and triangle data.
 * @example
 * // Create a new mesh supplying optional parameters using object literal notation
 * var mesh = cc3d.createMesh(
 *     graphicsDevice,
 *     positions,
 *     {
 *         normals: treeNormals,
 ...
 * @author Will Eastcott
 */
cc3d.createMesh = function (device, positions, opts) {
    ...
    if (normals !== null) {
        vertexDesc.push({semantic: cc3d.SEMANTIC_NORMAL, components: 3, type: cc3d.ELEMENTTYPE_FLOAT32});
    }
    if (tangents !== null) {
        vertexDesc.push({semantic: cc3d.SEMANTIC_TANGENT, components: 4, type: cc3d.ELEMENTTYPE_FLOAT32});
    }
    if (colors !== null) {
        vertexDesc.push({semantic: cc3d.SEMANTIC_COLOR, components: 4, type: cc3d.ELEMENTTYPE_UINT8, normalize: true});
    }
    if (uvs !== null) {
        vertexDesc.push({semantic: cc3d.SEMANTIC_TEXCOORD0, components: 2, type: cc3d.ELEMENTTYPE_FLOAT32});
    }
    if (uvs1 !== null) {
        vertexDesc.push({semantic: cc3d.SEMANTIC_TEXCOORD1, components: 2, type: cc3d.ELEMENTTYPE_FLOAT32});
    }
    var vertexFormat = new cc3d.VertexFormat(device, vertexDesc);

    // Create the vertex buffer
    var numVertices = positions.length / 3;
    var vertexBuffer = new cc3d.VertexBuffer(device, vertexFormat, numVertices);

    // Write the vertex data into the vertex buffer
    var iterator = new cc3d.VertexIterator(vertexBuffer);
    for (var i = 0; i < numVertices; i++) {
        iterator.element[cc3d.SEMANTIC_POSITION].set(positions[i * 3], positions[i * 3 + 1], positions[i * 3 + 2]);
        if (normals !== null) {
            iterator.element[cc3d.SEMANTIC_NORMAL].set(normals[i * 3], normals[i * 3 + 1], normals[i * 3 + 2]);
        }
        if (tangents !== null) {
            iterator.element[cc3d.SEMANTIC_TANGENT].set(tangents[i * 4], tangents[i * 4 + 1], tangents[i * 4 + 2], tangents[i * 4 + 3]);
        }
        if (colors !== null) {
            iterator.element[cc3d.SEMANTIC_COLOR].set(colors[i * 4], colors[i * 4 + 1], colors[i * 4 + 2], colors[i * 4 + 3]);
        }
        if (uvs !== null) {
            iterator.element[cc3d.SEMANTIC_TEXCOORD0].set(uvs[i * 2], uvs[i * 2 + 1]);
        }
        if (uvs1 !== null) {
            iterator.element[cc3d.SEMANTIC_TEXCOORD1].set(uvs1[i * 2], uvs1[i * 2 + 1]);
        }
        iterator.next();
    }
    iterator.end();
    ...
    if (indexed) {
        indexBuffer = new cc3d.IndexBuffer(device, cc3d.INDEXFORMAT_UINT16, indices.length);
    ...
    mesh.primitive[0].type = cc3d.PRIMITIVE_TRIANGLES;
 ...
 * @returns {cc3d.Mesh} A new torus-shaped mesh.
 * @author Will Eastcott
 */
cc3d.createTorus = function (device, opts) {
    // Check the supplied options and provide defaults for unspecified ones
    ...
    if (cc3d.precalculatedTangents) {
        options.tangents = cc3d.calculateTangents(positions, normals, uvs, indices);
    }

    return cc3d.createMesh(device, positions, options);
};

cc3d._createConeData = function (baseRadius, peakRadius, height, heightSegments, capSegments, roundedCaps) {
    // Variable declarations
    var i, j;
    var x, y, z, u, v;
    var pos = new cc.Vec3();
    var bottomToTop = new cc.Vec3();
    var norm = new cc.Vec3();
    ...
    if (height > 0) {
        for (i = 0; i <= heightSegments; i++) {
            for (j = 0; j <= capSegments; j++) {
                ...
                bottom = new cc.Vec3(sinTheta * baseRadius, -height / 2.0, cosTheta * baseRadius);
                top = new cc.Vec3(sinTheta * peakRadius, height / 2.0, cosTheta * peakRadius);
                pos.lerp(bottom, top, i / heightSegments);
                bottomToTop.sub2(top, bottom).normalize();
                tangent = new cc.Vec3(cosTheta, 0.0, -sinTheta);
 ...
 * @returns {cc3d.Mesh} A new cylinder-shaped mesh.
 * @author Will Eastcott
 */
cc3d.createCylinder = function (device, opts) {
    // Check the supplied options and provide defaults for unspecified ones
    ...
    var options = cc3d._createConeData(baseRadius, baseRadius, height, heightSegments, capSegments, false);

    if (cc3d.precalculatedTangents) {
        options.tangents = cc3d.calculateTangents(options.positions, options.normals, options.uvs, options.indices);
    }

    return cc3d.createMesh(device, options.positions, options);
};

/**
 * @function
 * @name cc3d.createCapsule
 ...
 * @author Will Eastcott
 */
cc3d.createCapsule = function (device, opts) {
    // Check the supplied options and provide defaults for unspecified ones
    ...
    // Create vertex data for a cone that has a base and peak radius that is the same (i.e. a cylinder)
    var options = cc3d._createConeData(radius, radius, height - 2 * radius, heightSegments, sides, true);
 ...
 * @returns {cc3d.Mesh} A new cone-shaped mesh.
 * @author Will Eastcott
 */
cc3d.createCone = function (device, opts) {
    // Check the supplied options and provide defaults for unspecified ones
    var baseRadius = opts && opts.baseRadius !== undefined ? opts.baseRadius : 0.5;
    ...
    var options = cc3d._createConeData(baseRadius, peakRadius, height, heightSegments, capSegments, false);
 ...
 * @returns {cc3d.Mesh} A new sphere-shaped mesh.
 * @author Will Eastcott
 */
cc3d.createSphere = function (device, opts) {
    // Check the supplied options and provide defaults for unspecified ones
 ...
 * @param {cc3d.GraphicsDevice} device The graphics device used to manage the mesh.
 * @param {Object} opts An object that specifies optional inputs for the function as follows:
 * @param {cc.Vec2v2} opts.halfExtents The half dimensions of the plane in the X and Z axes (defaults to [0.5, 0.5]).
 ...
 * @returns {cc3d.Mesh} A new plane-shaped mesh.
 * @author Will Eastcott
 */
cc3d.createPlane = function (device, opts) {
    // Check the supplied options and provide defaults for unspecified ones
    var he = opts && opts.halfExtents !== undefined ? opts.halfExtents : new cc.Vec2v2(0.5, 0.5);
 ...
 * @param {cc3d.GraphicsDevice} device The graphics device used to manage the mesh.
 * @param {Object} opts An object that specifies optional inputs for the function as follows:
 * @param {cc.Vec3} opts.halfExtents The half dimensions of the box in each axis (defaults to [0.5, 0.5, 0.5]).
 ...
 * @return {cc3d.Mesh} A new box-shaped mesh.
 * @author Will Eastcott
 */
cc3d.createBox = function (device, opts) {
    // Check the supplied options and provide defaults for unspecified ones
    var he = opts && opts.halfExtents !== undefined ? opts.halfExtents : new cc.Vec3(0.5, 0.5, 0.5);
    ...
    var generateFace = function (side, uSegments, vSegments) {
        ...
        for (i = 0; i <= uSegments; i++) {
            for (j = 0; j <= vSegments; j++) {
                var temp1 = new cc.Vec3();
                var temp2 = new cc.Vec3();
                var temp3 = new cc.Vec3();
                var r = new cc.Vec3();
