...
let precision = ['lowp', 'mediump', 'highp'];

// (HACKY) extract all builtin uniforms to the ignore list
let uniformIgnoreList = ['viewProj', 'model'];
...
function extractParams(tokens, cache, uniforms, attributes, extensions) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (dest === extensions) param.name = str.split(whitespaces)[1];
      ...
      let offset = 0;
      if (precision.indexOf(tokens[i+2].data) !== -1) {
        offset = 2;
      }
      param.name = tokens[i+offset+4].data; param.type = convertType(tokens[i+offset+2].data); 
    }
