...
const _touchStartHandler = function (touch, event: EventTouch) {
...
const _touchMoveHandler = function (touch, event: EventTouch) {
    const node = this.owner;
...
const _touchEndHandler = function (touch, event: EventTouch) {
    const pos = touch.getLocation();
    const node = this.owner;
...
const _touchCancelHandler = function (touch, event: EventTouch) {
    const node = this.owner;
...
const _mouseDownHandler = function (event: EventMouse) {
...
const _mouseMoveHandler = function (event: EventMouse) {
    const pos = event.getLocation();
    const node = this.owner;
...
const _mouseUpHandler = function (event: EventMouse) {
    const pos = event.getLocation();
    const node = this.owner;
...
const _mouseWheelHandler = function (event: EventTouch) {
    const pos = event.getLocation();
    const node = this.owner;
...
function _doDispatchEvent(owner: Node, event: EventTouch) {
    ...
    if (owner.eventProcessor.capturingListeners) {
        owner.eventProcessor.capturingListeners.emit(event.type, event);
    }
    if (!event.propagationImmediateStopped && owner.eventProcessor.bubblingListeners) {
        owner.eventProcessor.bubblingListeners.emit(event.type, event);
    }
...
function _checkListeners(listeners: EventTarget, events) {
    if (listeners) {
        for (let i = 0; i < events.length; ++i) {
            if (listeners.hasEventListener(events[i])) {
                return true;
            }
        }
    }
...
export class NodeEventProcessor {
    ...
    // Event listeners
    public bubblingListeners: EventTarget | null = null;
    public capturingListeners: EventTarget | null = null;
    ...
    public on(type: string, callback: Function, target?: Object, useCapture?: Object) {
        ...
        if (forDispatch) {
            ...
            //         break;
            // }
            if (!this.bubblingListeners) {
                this.bubblingListeners = new EventTarget();
            }
            return this.bubblingListeners.on(type, callback, target);
        }
    }
     ...
     */
    once(type: string, callback: Function, target: Object, useCapture?: Object) {
        ...
        if (forDispatch && useCapture) {
            listeners = this.capturingListeners = this.capturingListeners || new EventTarget();
        }
        else {
            listeners = this.bubblingListeners = this.bubblingListeners || new EventTarget();
        }
        ...
        if (!hasOnceListener) {
            ...
            let onceWrapper = function (arg1, arg2, arg3, arg4, arg5) {
                ...
                callback.call(this._node, arg1, arg2, arg3, arg4, arg5);
            };
    ...
    public _onDispatch(type: string, callback: Function, target?: Object, useCapture?: Object) {
    ...
    public off(type: string, callback: Function, target?: Object, useCapture?: Object) {
        ...
        if (touchEvent || mouseEvent) {
            ...
            if (touchEvent) {
                if (this.touchListener && !_checkListeners(this.touchListener, _touchEvents)) {
                    eventManager.removeListener(this.touchListener);
                    this.touchListener = null;
                }
            } else if (mouseEvent) {
                if (this.mouseListener && !_checkListeners(this.mouseListener, _mouseEvents)) {
                    eventManager.removeListener(this.mouseListener);
                    this.mouseListener = null;
                }
            }
        } else if (this.bubblingListeners) {
            this.bubblingListeners.off(type, callback, target);

            const hasListeners = this.bubblingListeners.hasEventListener(type);
    ...
    public _offDispatch(type: string, callback: Function, target?: Object, useCapture?: Object) {
        // Accept also patameters like: (type, callback, useCapture)
        if (typeof target === 'boolean') {
            useCapture = target;
            target = undefined;
        ...
        if (!callback) {
            this.capturingListeners && this.capturingListeners.removeAll(type);
            this.bubblingListeners && this.bubblingListeners.removeAll(type);
        } else {
            const listeners = useCapture ? this.capturingListeners : this.bubblingListeners;
            if (listeners) {
                listeners.remove(type, callback, target);

                if (target && target.__eventTargets) {
                    array.fastRemove(target.__eventTargets, this);
                }
            }
    ...
    public emit(type, ...args: any[]) {
        if (this.bubblingListeners) {
            this.bubblingListeners.emit(type, ...args);
        }
    }
    ...
    _getCapturingTargets(type, array) {
        ...
        while (parent) {
            if (parent.eventProcessor.capturingListeners && parent.eventProcessor.capturingListeners.hasEventListener(type)) {
                array.push(parent);
            }
    ...
    _getBubblingTargets(type, array) {
        var parent = this._node.parent;
        while (parent) {
            if (parent.eventProcessor.bubblingListeners && parent.eventProcessor.bubblingListeners.hasEventListener(type)) {
                array.push(parent);
            }
            parent = parent.parent;
        }
