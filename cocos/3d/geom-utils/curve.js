...
export class AnimationCurve {
    ...
    evaluate_slow(time) {
        ...
        let wrapMode = time < 0 ? this.preWrapMode : this.postWrapMode;
        let startTime = this.keyFrames[0].time;
        let endTime = this.keyFrames[this.keyFrames.length - 1].time;
        switch (wrapMode) {
            case WrapMode.Loop:
        ...
        let keyframe0 = this.keyFrames[preKFIndex];
        let keyframe1 = this.keyFrames[preKFIndex + 1];
    ...
    calcOptimizedKey(optKey, leftIndex, rightIndex) {
        let lhs = this.keyFrames[leftIndex];
        let rhs = this.keyFrames[rightIndex];
    ...
    findIndex(optKey, t) {
        ...
        if (cachedIndex !== -1) {
            let cachedTime = this.keyFrames[cachedIndex].time;
            if (t > cachedTime) {
