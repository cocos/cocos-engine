...
export function playOneShot (clip: AudioClip, volumeScale: number) {
    const sourceNode = audioContextManager.defaultContext.createSourceNode(audioBufferManager.getCache(clip.nativeUrl));
    sourceNode.connect(audioContextManager.defaultContext.destination);
    sourceNode.volume = volumeScale;
    sourceNode.onEnded(() => {
...
export class AudioPlayer extends DynamicPath<AudioState, AudioAction> implements Playable {
    ...
    // Common members
    _node: AudioState = AudioState.INIT;
    ...
    private _sourceNode: CCSourceNode;
    private _stereo: CCStereoPannerNode;
    ...
    constructor (clip: AudioClip, options?: PlayerOptions) {
        ...
        this._stereo = audioContextManager.defaultContext.createStereoPanner();
        this._stereo.connect(audioContextManager.defaultContext.destination);
    ...
    private _onShow () {
        ...
        if (this._node === AudioState.INTERRUPTED) {
            // No path should exist once it's alINIT paused.
