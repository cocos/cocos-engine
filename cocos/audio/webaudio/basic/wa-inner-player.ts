...
import { enqueueOperation, OperationInfo, OperationQueueable } from '../../base';
import AudioTimer from './audio-timer';
import { waAudioBufferManager } from '../graph/audio-buffer-manager';
    ...
    public play (): void {
        ...
        audioContextAgent!.runContext().then(() => {
            ...
            this._currentTimer = window.setTimeout(() => {
                waAudioBufferManager.tryReleasingCache(this._url);
                this.onEnd?.();
            }, this._duration * 1000);
        }).catch((e) => {});
    }
...
export class WAAudioPlayer implements OperationQueueable {
    ...
    destroy () {
        ...
        waAudioBufferManager.tryReleasingCache(this._src);
    ...
    static loadNative (url: string): Promise<AudioBuffer> {
        return new Promise((resolve, reject) => {
            const cachedAudioBuffer = waAudioBufferManager.getCache(url);
            if (cachedAudioBuffer) {
                waAudioBufferManager.retainCache(url);
            ...
            xhr.onload = () => {
                if (xhr.status === 200 || xhr.status === 0) {
                    audioContextAgent!.decodeAudioData(xhr.response).then((decodedAudioBuffer) => {
                        waAudioBufferManager.addCache(url, decodedAudioBuffer);
