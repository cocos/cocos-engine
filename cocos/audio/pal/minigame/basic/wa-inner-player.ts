...
export class WAAudioPlayer implements OperationQueueable {
    private _src: string;
    ...
    static load (url: string): Promise<WAAudioPlayer> {
        return new Promise((resolve) => {
            WAAudioPlayer.loadNative(url).then((audioBuffer) => {
                resolve(new WAAudioPlayer(audioBuffer, url));
            }).catch((e) => {});
        });
    }
    ...
    static loadOneShotAudio (url: string, volume: number): Promise<WAOneShotAudio> {
        return new Promise((resolve, reject) => {
            WAAudioPlayer.loadNative(url).then((audioBuffer) => {
                // @ts-expect-error AudioPlayer should be a friend class in OneShotAudio
                const oneShotAudio = new WAOneShotAudio(audioBuffer, volume, url);
