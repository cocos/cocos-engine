...
import { enqueueOperation, BaseInnerAudioPlayer, BaseInnerOneShotAudio, OperationInfo } from '../../base';
...
export class DomInnerOneShotAudio extends BaseInnerOneShotAudio {
...
export class DomInnerAudioPlayer extends BaseInnerAudioPlayer {
    private _domAudio: HTMLAudioElement;
    ...
    static load (url: string): Promise<DomInnerAudioPlayer> {
        return new Promise((resolve) => {
            DomInnerAudioPlayer.loadNative(url).then((domAudio) => {
                const player = new DomInnerAudioPlayer(domAudio);
                resolve(player);
            }).catch((e) => {});
        });
    }
    ...
    static loadOneShotAudio (url: string, volume: number): Promise<DomInnerOneShotAudio> {
        return new Promise((resolve, reject) => {
            DomInnerAudioPlayer.loadNative(url).then((domAudio) => {
                // @ts-expect-error AudioPlayer should be a friend class in OneShotAudio
                const oneShotAudio = new DomInnerOneShotAudio(domAudio, volume);
