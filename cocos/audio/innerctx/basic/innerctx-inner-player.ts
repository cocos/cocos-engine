...
import { enqueueOperation, BaseInnerAudioPlayer, OperationInfo } from '../../base';
import { EventTarget } from '../../../core/event';
import { AudioEvent, AudioPCMDataView, AudioState, AudioType } from '../../type';
import { clamp, clamp01 } from '../../../core';

export class InnerCtxOneShotAudio {
...
export class InnerCtxAudioPlayer extends BaseInnerAudioPlayer {
    private _innerAudioContext: InnerAudioContext;
    ...
    protected _offEvent (eventName: string) {
    ...
    static load (url: string): Promise<InnerCtxAudioPlayer> {
        return new Promise((resolve) => {
            InnerCtxAudioPlayer.loadNative(url).then((innerAudioContext) => {
                resolve(new InnerCtxAudioPlayer(<InnerAudioContext>innerAudioContext));
            }).catch((e) => {});
        });
    }
    ...
    static loadOneShotAudio (url: string, volume: number): Promise<InnerCtxOneShotAudio> {
        return new Promise((resolve, reject) => {
            InnerCtxAudioPlayer.loadNative(url).then((innerAudioContext) => {
                // @ts-expect-error AudioPlayer should be a friend class in OneShotAudio
                resolve(new InnerCtxOneShotAudio(innerAudioContext, volume));
            }).catch(reject);
        });
    }
