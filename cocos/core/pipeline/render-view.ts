...
export class RenderView {
    ...
    public set priority (val: number) {
        ...
        if (legacyGlobalExports.director.root) {
            legacyGlobalExports.director.root.sortViews();
        }
    }
    ...
    public setExecuteFlows (flows: string[] | undefined) {
        ...
        if (flows && flows.length === 1 && flows[0] === 'UIFlow') {
            this._flows.push(legacyGlobalExports.director.root.pipeline.getFlow('UIFlow'));
            return;
        }
        const pipelineFlows = legacyGlobalExports.director.root.pipeline.activeFlows;
