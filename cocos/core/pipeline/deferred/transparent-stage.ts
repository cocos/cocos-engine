...
import { GFXLoadOp } from '../../gfx/define';
 ...
 * @zh 前向渲染阶段。
 */
@ccclass('TransparentStage')
export class TransparentStage extends RenderStage {

    public static initInfo: IRenderStageInfo = {
        name: 'TransparentStage',
        priority: DeferredStagePriority.TRANSPARENT,
        tag: 0,
        renderQueues: [
            {
                isTransparent: true,
    ...
    private _phaseID = getPhaseID('deferred-transparent');
    ...
    public render (view: RenderView) {
        ...
        //FIXEME: ugly code
        let renderPass_temp = renderPass;
        renderPass_temp.colorAttachments[0].loadOp = GFXLoadOp.LOAD;

        cmdBuff.beginRenderPass(renderPass_temp, framebuffer, this._renderArea!,
            colors, camera.clearDepth, camera.clearStencil);

        cmdBuff.bindDescriptorSet(SetIndex.GLOBAL, pipeline.descriptorSet);

        for (let i = 0; i < this.renderQueues.length; i++) {
            this._renderQueues[i].recordCommandBuffer(device, renderPass_temp, cmdBuff);
        }
        this._instancedQueue.recordCommandBuffer(device, renderPass_temp, cmdBuff);
        this._batchedQueue.recordCommandBuffer(device, renderPass_temp, cmdBuff);
        //this._additiveLightQueue.recordCommandBuffer(device, renderPass_temp, cmdBuff);
