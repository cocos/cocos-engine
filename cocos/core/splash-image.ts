...
import { GFXCommandBufferType, GFXBufferUsageBit, GFXClearFlag, IGFXRect, GFXTextureType, GFXTextureUsageBit, GFXFormat, GFXTextureViewType, GFXMemoryUsageBit, GFXBufferTextureCopy } from "./gfx";
import { GFXInputAssembler, IGFXAttribute } from "./gfx/input-assembler";
import { GFXBuffer } from "./gfx/buffer";
import { GFXPipelineState } from "./gfx/pipeline-state";
import { GFXFramebuffer } from "./gfx/framebuffer";
import { Material, EffectAsset } from "./assets";
...
class SplashImage {
    ...
    public main (device: GFXDevice, cb: Function) {
        ...
        if (this.setting.totalTime <= 0) {
            ...
            delete cc.splashImage;
            return;
        } else {
            director.on(Director.EVENT_INIT, this.hide, this);
            this.device = device;
            this.callBack = cb;
    ...
    private init () {
        ...
        const animate = (time: number) => {
            ...
            if (elapsedTime > this.setting.totalTime) {
                if (this.callBackFlag == false && this.callBack != null) {
                    this.callBack();
...
cc.splashImage = new SplashImage();