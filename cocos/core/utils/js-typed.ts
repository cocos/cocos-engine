    ...
    for (const source of sources) {
        if (source) {
            if (typeof source !== 'object') {
                legacyGlobalExports.errorID(5402, source);
                continue;
            }
...
export function mixin (object?: any, ...sources: any[]) {
    ...
    for (const source of sources) {
        if (source) {
            if (typeof source !== 'object') {
                legacyGlobalExports.errorID(5403, source);
                continue;
            }
            for (const name in source) {
                _copyprop(name, source, object);
...
export function extend (cls: Function, base: Function) {
    if (DEV) {
        if (!base) {
            legacyGlobalExports.errorID(5404);
            return;
        }
        if (!cls) {
            legacyGlobalExports.errorID(5405);
            return;
        }
        if (Object.keys(cls.prototype).length > 0) {
            legacyGlobalExports.errorID(5406);
        }
    }
