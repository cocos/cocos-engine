...
export class RenderTexture extends TextureBase {
    ...
    public destroy () {
        if (this._window) {
            legacyGlobalExports.director.root!.destroyWindow(this._window);
            this._window = null;
        }
    ...
    protected _createWindow (device: GFXDevice) {
        ...
        this._window = legacyGlobalExports.director.root!.createWindow(config);
        this.loaded = true;
        this.emit('load');
    }
}

legacyGlobalExports.RenderTexture = RenderTexture;
