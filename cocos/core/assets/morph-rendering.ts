        ...
        for (let iAttribute = 0; iAttribute < this._attributes.length; ++iAttribute) {
            ...
            if (false) {
                for (let i = 0; i <myAttribute.local.length; ++i) {
                    if (i % 3 === 1) {
                        myAttribute.local[i] = (legacyGlobalExports.director.getTotalFrames() % 500) * 0.001;
                    } else {
...
class MorphUniforms {
    ...
    public setWeights (weights: number[]) {
        ...
        for (let iWeight = 0; iWeight < weights.length; ++iWeight) {
            this._localBuffer.setFloat32(UBOMorph.OFFSET_OF_WEIGHTS + 4 * iWeight, weights[iWeight], legacyGlobalExports.sys.isLittleEndian);
        }
    }

    public setMorphTextureInfo (width: number, height: number) {
        this._localBuffer.setFloat32(UBOMorph.OFFSET_OF_DISPLACEMENT_TEXTURE_WIDTH, width, legacyGlobalExports.sys.isLittleEndian);
        this._localBuffer.setFloat32(UBOMorph.OFFSET_OF_DISPLACEMENT_TEXTURE_HEIGHT, height, legacyGlobalExports.sys.isLittleEndian);
    }
