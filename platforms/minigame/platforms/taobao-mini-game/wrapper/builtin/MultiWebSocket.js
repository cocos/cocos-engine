...
export default class MultiWebSocket {
  ...
  constructor(url, protocols = []) {
    ...
    const socketTask = my.connectSocket({
      ...
      multiple: true,
      fail: (res) => {
        console.warn("MultiWebSocket connectSocket fail",res);
      }
    })
  ...
  send(data) {
    const noStr = typeof data !== 'string';
    if (noStr && !(data instanceof ArrayBuffer) && !ArrayBuffer.isView(data)) {
    ...
    this._socketTask.send({
      ...
      isBuffer,
      fail: (res) => {
        console.warn("MultiWebSocket send fail",res);
      }
    })
  }
}